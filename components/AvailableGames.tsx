...
const SectionHeader: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <h2 className="text-3xl md:text-4xl font-bold text-red-400 tracking-widest uppercase mb-8 relative pb-4 after:content-[''] after:absolute after:left-0 after:bottom-0 after:w-24 after:h-1 after:bg-red-500">
    {children}
  </h2>
);
...
const AvailableGames: React.FC = () => {
  const [approvedGames, setApprovedGames] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const games = snapshot.docs
        ...
        .filter((game: any) => game.fileUrl); // Extra safety check
      setApprovedGames(games);
      setLoading(false);
    }, (error) => {
      console.error("Firestore error in AvailableGames:", error);
      setLoading(false);
    });
